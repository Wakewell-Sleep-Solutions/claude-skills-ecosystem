...
**If scraping healthcare-related data (patient reviews, provider directories, health records, insurance data):**
- Patient reviews on Healthgrades/Zocdoc/Google may contain PHI — storing or processing this data may create HIPAA obligations
- Never scrape patient portals, EHR systems, or any authenticated healthcare data without explicit authorization
- Scraped health data sent to LLM APIs (Gemini, etc.) without BAA = potential HIPAA violation
- Web scraping of healthcare sites may violate CFAA if terms of service prohibit automated access
- If enriching scraped data with patient-identifiable information, full HIPAA safeguards apply
- De-identification: remove all 18 Safe Harbor identifiers before storing/processing

> **Cross-reference:** `hipaa-compliance` for PHI framework and BAA requirements.
